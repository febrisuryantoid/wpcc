...
import { Palette, CheckCircle, Sparkles, Sliders, Layout, Move, HelpCircle } from 'lucide-react';

export const Slide35: React.FC<SceneProps> = (props) => {
  if (!props.isActive) return null;

  return (
    <SlideAnimationProvider scene={props.scene} isActive={props.isActive}>
      <Slide35Content {...props} />
    </SlideAnimationProvider>
  );
};

const Slide35Content: React.FC<SceneProps> = ({ scene }) => {
  const { 
    headingShowMode, 
    descriptionShowMode, 
    headingDuration, 
